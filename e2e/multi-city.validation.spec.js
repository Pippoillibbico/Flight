...
test('show empty-field errors only after search attempt without technical IATA copy', async ({ page }) => {
  await bootMultiCity(page, { language: 'it' });

  await expect(page.getByTestId('segment-origin-error-0')).toHaveCount(0);
  await page.getByTestId('submit-search-action').click();

  await expect(page.getByTestId('segment-origin-error-0')).toHaveText('Inserisci la partenza.');
  await expect(page.getByTestId('segment-origin-error-0')).not.toContainText('IATA');
});
