...
export function getDefaultFreeLimits() {
  return {
    daily_search_limit: Number(process.env.FREE_DAILY_SEARCH_LIMIT || 3),
    active_alert_limit: Number(process.env.FREE_ACTIVE_ALERT_LIMIT || 1),
    demo_daily_search_limit: Number(process.env.DEMO_DAILY_SEARCH_LIMIT || 2),
    demo_minute_ip_limit: Number(process.env.DEMO_MINUTE_IP_LIMIT || 5),
    demo_minute_device_limit: Number(process.env.DEMO_MINUTE_DEVICE_LIMIT || 6),
    user_minute_limit: Number(process.env.FREE_USER_MINUTE_LIMIT || 8)
  };
}
